        ...
        <div id="homeBar" class="">
            @if(count($directory) > 0)
                <div class="text-center text-2xl mb-8">
                    Borno Women Development Initiative (BOWDI) <br> <b>Directories</b>
                </div>
                <div class="my-2">
                            ...
                            @foreach($directory as $item)
                                <tr class="text-left">
                                    ...
                                    @if(Auth::user()->type == 1)
                                        <!-- Action  -->
                                        <td class="flex px-2">
                                            <form action="{{ route('directory-edit', $item->id) }}">
                                                @csrf 
                                            ...
                                            &nbsp;&nbsp;
                                            <form action="{{ route('directory-delete', $item->id) }}" method="POST">
                                                @csrf 
                                                @method('DELETE')
                                                <button class="relative top-2">
