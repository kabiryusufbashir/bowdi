        ...
        <div id="homeBar" class="">
            @if(count($doc) > 0)
                <div class="text-center text-2xl mb-8">
                    Borno Women Development Initiative (BOWDI) <br> <b>Documents</b>
                </div>
                <div class="my-2">
                        ...
                        <tbody>
                            ...
                            @foreach($doc as $item)
                                <tr class="text-left">
                                    ...
                                    <td>
                                        <a target="_blank" href="{{ asset('assets/documents/'.$item->path) }}">{{ $item->name }}</a>
                                    </td>
                                    <td>
                                        <a target="_blank" href="{{ asset('assets/documents/'.$item->path) }}">{{ $item->description }}</a>
                                    </td>
                                    <td>
                                        {{ App\Models\Directory::where('id', $item->category)->pluck('name')->first() }}
                                    </td>
                                    <td>
                                    ...
                                    @if(Auth::user()->type == 1)
                                        <!-- Action  -->
                                        <td class="flex px-2">
                                            <form action="{{ route('doc-edit', $item->id) }}">
                                                @csrf 
                                            ...
                                            &nbsp;&nbsp;
                                            <form action="{{ route('doc-delete', $item->id) }}" method="POST">
                                                @csrf 
                                                @method('DELETE')
                                                <button class="relative top-2">
