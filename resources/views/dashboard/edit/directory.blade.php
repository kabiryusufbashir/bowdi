...
@section('container')
    ...
    <div class="col-span-10">
        ...
        <div class="lg:w-2/3 w-full mx-auto shadow-lg">
            <form action="{{ route('directory-update', $directory->id) }}" method="POST" class="px-6 lg:px-8 py-8">
                @csrf
                @method('PATCH')
                    <div class="lg:grid grid-cols-2 gap-4 items-center">
                        <div>
                            <label for="name" class="text-lg font-medium">Directory Name</label><br>
                            <input type="text" name="name" value="{{$directory->name}}" placeholder="directory Name" class="input-field">
                            @error('name')
                                {{$message}}
                            @enderror
                        </div>
                        <div class="text-center">
                            <button class="submit-button">Edit Directory</button>
                        </div>    
                    </div>
            </form>
        </div>
